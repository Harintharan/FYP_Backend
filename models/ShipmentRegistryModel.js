...
,
  ...
  async getShipmentById(shipment_id) {
    const result = await pool.query(
      `SELECT * FROM shipmentRegistry WHERE shipment_id=$1`,
      [shipment_id]
    );
    return result.rows[0];
  },
  ...
  async getAllShipments() {
    const result = await pool.query(
      `SELECT * FROM shipmentRegistry ORDER BY created_at DESC`
    );
    return result.rows;
  },
