 ...
 */
export async function getSupplierDashboard(req, res) {
  try {
    const userId = req.userId;
    const userUUID = req.registration?.id;
    ...
    const stats = await getSupplierDashboardStats(userUUID, userId);
    return res.status(200).json(stats);
  } catch (err) {
    return handleControllerError(res, err, {
      logMessage: "Error fetching supplier dashboard",
      fallbackMessage: "Unable to fetch dashboard statistics",
    });
  }
}
