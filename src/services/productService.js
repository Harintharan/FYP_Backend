...
export async function createProduct({ payload, registration, wallet }) {
  ...
  const record = await insertProduct({
    ...
    requiredStorageTemp: normalized.requiredStorageTemp ?? null,
    transportRoutePlanId: normalized.transportRoutePlanId ?? null,
    handlingInstructions: normalized.handlingInstructions ?? null,
    expiryDate: normalized.expiryDate ?? null,
    sensorDeviceUUID: normalized.sensorDeviceUUID ?? null,
    microprocessorMac: normalized.microprocessorMac ?? null,
    sensorTypes: normalized.sensorTypes ?? null,
    qrId: normalized.qrId ?? null,
    wifiSSID: normalized.wifiSSID ?? null,
    encryptedWifiPassword,
    manufacturerUUID: normalized.manufacturerUUID,
    originFacilityAddr: normalized.originFacilityAddr ?? null,
